...
swift test                      # 607 parity checks + 9 model behaviour cases
...
Glaze app — 607 checks. **The UI is largely built**: an application menu, a menu bar item,
...
HTML, plus full backups. Counted against the reference's own router, this port has **19 of its 20 routes**: what is
left is Canvas — plus PDF export, and signing, which is blocked on a

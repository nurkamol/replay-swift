...
swift test                      # 600 parity checks + 9 model behaviour cases
...
Glaze app — 600 checks. **The UI is largely built**: an application menu, a menu bar item,
...
HTML, plus full backups. Counted against the reference's own router, this port has **17 of its 20 routes**: what is
left is Canvas and a museum — plus PDF export, and signing, which is blocked on a

...
| Update check and install | done | **this port's own — the reference updates through the Glaze Store.** Opt-in and off by default; at most one `GET` a day to GitHub's public releases API, no body and no identifier, and a dismissible bar with the notes if a newer tag exists. It installs, as of 0.9.3: the zip, the SHA-256 published beside it, `ditto`, a signature and identity and version check, then `replaceItemAt` and relaunch. The trust is HTTPS to a repository named in the source plus that checksum — the Homebrew-formula model, weaker than notarisation, and said so in the README. It refuses for a Homebrew copy, a translocated one, or a read-only location. As of 0.9.5 a refused check is not counted as the day's check, the `x-ratelimit-reset` GitHub names is honoured rather than guessed at, and the request is conditional — which saves bytes and, measured, no rate limit at all (`docs/FINDINGS.md`). A launch that finds itself on a newer version than the last one says so — the What's New window when the updater restarted it, a dismissible banner when it arrived by any other means, and nothing at all for a downgrade (`Updates.launchNote`). The comparison lives in `Updates` and is tested; the request lives in `UpdateModel`. README and SPEC were amended rather than defended — see the divergences |
...
| Replay Story | done | the camera travels through a memory and the heaviest five things around it, dwelling on each and coming home — narration by motion, no words. The path is `tourPath`, ordered as the reference orders it and tie-broken explicitly; the camera's numbers are contract-checked. The line drawing itself between stops and the breath each stop lets out are **this port's own** — see the divergences |

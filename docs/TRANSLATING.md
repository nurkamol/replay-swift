...
- **Rows beginning `plural:` are one form of a counted noun**, and there is one per plural
  category *your* language has. `plural:few:%@ session` is the form Russian uses for 2–4 and
  22–24; `plural:one:%@ session` is the form it uses for 1 and 21 but not 11. Fill every row —
  a noun missing any of its categories is dropped and falls back to two English-shaped forms.
  Keep the `%@`; it becomes the number.
...
## Counted nouns, and why `n == 1` is not a plural rule

`Loc.count(n, "%@ session", "%@ sessions")` used to pick the second form whenever `n` was not
1. That is English's rule, not *a* rule. Russian has four forms and 21 takes the same one as
1 while 11 takes neither; Arabic has six; Japanese has one. Every counted noun in a Russian
build would have been wrong, and no amount of translating the two English forms more
carefully could have fixed it — the bug is in the choosing, not the words.

So each noun becomes a `.stringsdict` entry with one form per category, and Foundation picks
between them using the locale's own CLDR rules:

```
uz.lproj/Replay.stringsdict   ← generated by `build`, one entry per counted noun
```

**The categories come from ICU, not from a list in this repo.** `tools/translate.mjs` asks
`Intl.PluralRules` — the same CLDR data Foundation uses at runtime — so `ar` gets six rows,
`ru` four, `ja` one, and nothing here can fall behind a CLDR release.

English ships no `.stringsdict` on purpose: its rule and `n == 1` are the same rule, and a
file restating it is one more thing to keep in step. `Loc.count` therefore still has its
two-form path, and it is not dead code — it is what a language falls back to for a noun whose
categories are not all filled in, so a half-finished `.stringsdict` degrades to the old
behaviour rather than to an empty string.

**Both English forms are still keys**, because that fallback needs them. `Loc.count`'s
arguments used to reach nothing at all: the key scanner reads `Loc.t("…")` and these arrive by
another call, so `%@ day`, `%@ visit`, `%@ visits`, `%@ switch` and `%@ switches` were absent
from the catalogue entirely and rendered in English in a table reporting 100%. That is the
third distinct way "scan the source for strings" has been wrong here — see the table above.
